...
class gqm():
    ...
    def fit(self, obs_data, sim_data):
        ...
        #: dry day fraction
        self.obs_param['c'] = (
            obs_data[obs_data == 0].shape[0]) / (obs_data.shape[0])
        self.sim_param['c'] = (
            sim_data[sim_data == 0].shape[0]) / (sim_data.shape[0])

        #: fit gamma with non zero values with floc=0
        self.obs_param['a'], _, self.obs_param['b'] = gamma.fit(
            obs_data[obs_data > 0], floc=0)
        self.sim_param['a'], _, self.sim_param['b'] = gamma.fit(
            sim_data[sim_data > 0], floc=0)
    ...
    def correct(self, simVal: float) -> float:
        ...
        simValCDF = gamma.cdf(
            simVal, a=self.sim_param['a'], scale=self.sim_param['b'], loc=0)
