...
def tank_discharge(
    ...
    if not utils.shape_alike(precipitation, evapotranspiration):
        ...
        raise ValueError(
            'ERROR-TANK-01: length mismatch between precipitation and evapotranspiration data')
    ...
    for t in np.arange(time_step):
        ...
        '''
        if t < time_step - 1:
            tank_storage[t+1, 0] = tank_storage[t, 0] + del_rf_et[t +
                                                                  1] - (side_outlet_flow[t, 0] + bottom_outlet_flow[t, 0])

            tank_storage[t+1, 1] = tank_storage[t, 1] + bottom_outlet_flow[t,
                                                                           0] - (side_outlet_flow[t, 1] + bottom_outlet_flow[t, 1])

            tank_storage[t+1, 2] = tank_storage[t, 2] + bottom_outlet_flow[t,
                                                                           1] - (side_outlet_flow[t, 2] + bottom_outlet_flow[t, 2])

            tank_storage[t+1, 3] = tank_storage[t, 3] + \
                bottom_outlet_flow[t, 2] - side_outlet_flow[t, 3]
        ...
        for i in range(4):
            ...
            total_tank_outflow = bottom_outlet_flow[t, i] + \
                side_outlet_flow[t, i] if i <= 2 else side_outlet_flow[t, i]

            if total_tank_outflow > tank_storage[t, i]:
                print(
                    f'WARNING-TANK-02: Total outlet flow exceeded tank storage for tank {i} at timestep {t}')
